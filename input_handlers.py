...
class BaseEventHandler(tcod.event.EventDispatch[ActionOrHandler]):
    def handle_events(self, event: tcod.event.Event) -> BaseEventHandler:
        """Handle an event and return the next handler to use"""
...
class PopupMessage(BaseEventHandler):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[BaseEventHandler]:
        """Return to the parent handler when any key is pressed"""
        return self.parent
...
class EventHandler(BaseEventHandler):
    ...
    def handle_action(self, action: Optional[Action]) -> bool:
        """Handle actions returned by the event handler"""
        if action is None:
            return False
...
class AskUserEventHandler(EventHandler):
    ...
    def ev_mousebuttondown(
        ...
        """By default any mouse button exits this input handler"""
        return self.on_exit()

    def on_exit(self) -> Optional[ActionOrHandler]:
        """Return to the main event handler"""
...
class InventoryEventHandler(AskUserEventHandler):
    """This handler lets a user select an item"""
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        if index >= 0 and index < len(player.inventory.items):
            try:
                selected_item = player.inventory.items[index]
            except IndexError:
                self.engine.message_log.add_message(
                    "Invalid item selected.", color.impossible
                )
                return None
    ...
    def on_item_selected(self, item: Item) -> Optional[ActionOrHandler]:
        """User has selected a valid item"""
        raise NotImplementedError()
...
class MainGameEventHandler(EventHandler):
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        # TODO: Help screen for input options
...
class GameOverEventHandler(EventHandler):
    def on_quit(self) -> None:
        """Handle exiting a game from game over state"""
        path = Path("savegame.sav")

        if path.exists():
            path.unlink()  # Delete the save file

        raise exceptions.QuitWithoutSaving()  # Quit the game without saving
